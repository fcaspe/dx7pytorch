                               ...
                               const char *value)
{
    ...
    if (section < 0 || section > 3)
        {printf("patch configuration failed: invalid section '%c'", key[7]);
        return NULL;
        }
    ...
    {
        printf("patch configuration failed: corrupt data");
        return NULL;
    }
                                   ...
                                   const char *value)
{
    ...
    if (!strcmp(value, "off")) {
        ...
        if (!decode_7in6(value, sizeof(edit_buffer), (uint8_t *)&edit_buffer)) 
            {
            printf("patch edit failed: corrupt data");
            return NULL;
            }
                                   ...
                                   const char *value)
{
    ...
    if (!decode_7in6(value, DX7_PERFORMANCE_SIZE, instance->performance_buffer)) 
        {
        printf("performance edit failed: corrupt data");
        return NULL;
        }
...
hexter_instance_handle_monophonic(hexter_instance_t *instance, const char *value)
{
    ...
    if (mode == -1) {
        printf("error: monophonic value not recognized");
        return NULL;
    }
...
hexter_instance_handle_polyphony(hexter_instance_t *instance, const char *value)
{
    ...
    if (polyphony < 1 || polyphony > HEXTER_MAX_POLYPHONY) {
        printf("error: polyphony value out of range");
        return NULL;
    }
    /* set the new limit */
